...
const Jobs = () => {
  ...
  const fetchSkills = async () => {
    try {
      const response = await axios.get('http://localhost:5070/api/getAllSkills');
  ...
  const getSkillsByIds = async(record) => {
    ...
    const skillNames = record.map(id => {
  ...
  const deleteJob = async (record) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:5070/api/job_roles/${record.job_id}`);
  ...
  const columns = [
    ...
    //   key: 'skills_ids',
    //   render: (record) =>getSkillsByIds(record),
    // },
