...
app.delete('/api/parsed_cv_data/:id', async (req, res) => {
    const id = req.params.id;

    try {
        // Delete data from the job_roles table
        const result = await itemsPool.query(
            'DELETE FROM parsed_cv_data WHERE id = $1 RETURNING *',
            [id]
        );
...
app.post("/api/preprocess", async (req, res) => {
    try {
      ...
      if (response.status !== 200) {
        return res.status(500).json({ error: "Failed to fetch the content." });
      }
      ...
      res.status(500).json({ error: "An error occurred during preprocessing." });
    }
  });
...
function preprocessText(rawText) {
    ...
    let cleanText = rawText.replace(/[^\w\s]/g, " ").replace(/\s+/g, " ").trim();
  
    // Tokenize and normalize text
    const tokenizer = new natural.WordTokenizer();
    const tokens = tokenizer.tokenize(cleanText);
    const normalizedText = tokens.map((token) => token.toLowerCase()).join(" ");
